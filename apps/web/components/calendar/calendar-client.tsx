...
export function CalendarClient({ initialErrorMessage = null }: CalendarClientProps) {
	...
	// Listen for aside-panel actions (edit, delete, pin) dispatched by EventDetailView
	useEffect(() => {
		const onAsideEdit = (rawEvent: Event) => {
			const { eventId } = (rawEvent as CustomEvent<{ eventId?: string }>).detail ?? {};
			if (!eventId) return;
			const matched = scheduleEventsByIdRef.current.get(eventId);
			if (matched) openEditorForEventRef.current(matched, "edit");
		};
		const onAsideDelete = (rawEvent: Event) => {
			const { eventId } = (rawEvent as CustomEvent<{ eventId?: string }>).detail ?? {};
			if (!eventId) return;
			void deleteEventMutation({ id: eventId as Id<"calendarEvents">, scope: "single" });
			void pushEventToGoogle({
				eventId: eventId as Id<"calendarEvents">,
				operation: "delete",
				scope: "single",
			}).catch(() => {});
		};
		const onAsidePin = (rawEvent: Event) => {
			const { eventId, pinned } =
				(rawEvent as CustomEvent<{ eventId?: string; pinned?: boolean }>).detail ?? {};
			if (!eventId) return;
		...
		window.addEventListener("calendar:event-edit", onAsideEdit);
		window.addEventListener("calendar:event-delete", onAsideDelete);
		window.addEventListener("calendar:event-pin", onAsidePin);
		return () => {
			window.removeEventListener("calendar:event-edit", onAsideEdit);
			window.removeEventListener("calendar:event-delete", onAsideDelete);
					...
					{editor ? (
									...
									<div className="grid grid-cols-[20px_1fr] items-start gap-3">
										...
										{isDetailsMode ? (
											...
											<div className="space-y-2">
												...
												<div className="font-[family-name:var(--font-cutive)] text-[0.52rem] tracking-[0.1em] text-muted-foreground/80">
						...
						<>
							<div className="rounded-xl border border-dashed border-border/60 bg-card/40 p-4 text-center">
								...
								<div className="mt-1 font-[family-name:var(--font-cutive)] text-[0.52rem] tracking-[0.1em] text-muted-foreground/80">
		...
		<>
			<div className="h-full min-h-0 overflow-hidden bg-background text-foreground flex flex-col gap-0">
				<CalendarTopbar
					...
					onToday={() => setDateValue(formatDateInput(new Date()))}
					...
					onCreateHabit={() => setQuickCreateHabitOpen(true)}
					onSync={() => {
						void syncNow("manual");
					}}
					...
					onSelectDate={(date) => setDateValue(formatDateInput(date))}
				...
				<CalendarRecurringMoveDialog
					open={Boolean(pendingMoveUpdate)}
					title={pendingMoveUpdate?.title ?? null}
					onOpenChange={(open) => {
						if (!open) setPendingMoveUpdate(null);
					}}
					onCancel={() => setPendingMoveUpdate(null)}
					onMoveSingle={() => {
						void moveRecurringEvent("single");
					}}
					onMoveSeries={() => {
