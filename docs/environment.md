...
| `INTERNAL_ADMIN_USER_IDS` | Comma-separated WorkOS user IDs allowed to run internal-only tools (for example `seedDevTasks`) | No |
...
- Env validation is web-local and centralized in `apps/web/env/*`.
- `bun run env:check` executes `apps/web/env/check.ts` to validate required variables before builds.
...
- Convex-only secrets like `WORKOS_WEBHOOK_SECRET` still need to be configured in Convex separately.
